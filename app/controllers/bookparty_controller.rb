class BookpartyController < ApplicationController
  ...
  def storebookinfo_complete
    ...
    sellbook.bookinitprice = params[:bookprice]
  ...
  def exact_buy
    ...
    sellbook.save
    auction = Auction.where(user_id: session[:user_id], sellbook_id: params[:id]).take
    if auction.nil?
      auction_new = Auction.new #새로 auction기록을 남기고 저장
      auction_new.user_id = session[:user_id]
      auction_new.sellbook_id = params[:id]
      auction_new.auctionprice = sellbook.nowbookprice
      auction_new.finished = 0
      auction_new.save
    else
  ...
  def auction_delete
    @auctiondelete = Auction.where(sellbook_id: params[:id], user_id: session[:user_id]).take
    unless @auctiondelete.nil?
      @auctiondelete.destroy
    end

    remain_auction = Auction.where(sellbook_id: params[:id]).order('auctionprice DESC').take
    sellbook = Sellbook.find(params[:id])
    if remain_auction.nil?
      sellbook.bookprice = sellbook.bookinitprice
      sellbook.save
    else
      sellbook.bookprice = remain_auction.auctionprice
      sellbook.save
    end
